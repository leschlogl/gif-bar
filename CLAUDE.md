...
**Current status**: all of PRD milestones 1–8 are done, including real Networking (milestone 3). `Networking` implements `APIClient`/`Endpoint`/`RequestBuilder`/`NetworkError`, the Giphy DTO decoder, `GifRenditionPicker`, and `ImageCache`. `Services.GiphyService: GifProviding` is the real implementation, wired in at `App/GIFBarApp.swift`'s composition root (replacing the earlier `MockGifProvider`, which still exists and is used by tests/previews). Because `ViewModels` cannot import `Persistence`/`Networking` directly, `Services.FavoritesService` wraps a `Persistence.FavoritesStore`, and `Services.ImageDataLoading` (with `Networking.ImageCache` conforming via a retroactive extension) is what `GifBarViewModel` depends on for both cases.
